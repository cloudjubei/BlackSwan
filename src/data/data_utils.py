...
def plot_actions_data_regression(env):
    
    # end_point = 26 * 24 * 60
    end_point = 3 * 28 * 24 * 60
    actions = env.predictions[:end_point]
    targets = env.actuals[:end_point]
    ...
    result_df['rewards_profitable_strong'] = np.array([1 if a == 1 else np.nan for a in rewards_profitable]) * (result_df['low'] -600)
    result_df['rewards_profitable_ok'] = np.array([1 if a >= 2 and a < buyreward_maxwait else np.nan for a in rewards_profitable]) * (result_df['low'] -600)

    result_df['rewards_pos'] = np.array([1 if a > 0 else np.nan for a in targets])
    result_df['rewards_neg'] = np.array([1 if a <= 0 else np.nan for a in targets])
    result_df['actions_ok'] = np.array([1 if a > 0 else np.nan for a in actions]) * result_df['rewards_pos'] * (result_df['low'] -200)
    result_df['actions_bad'] = np.array([1 if a > 0 else np.nan for a in actions]) * result_df['rewards_neg'] * (result_df['high'] +200)
